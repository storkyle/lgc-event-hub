...
export function validateCreateSubscriber(
  ...
  if (delivery_guarantee !== undefined) {
    if (!["at_least_once", "at_most_once"].includes(delivery_guarantee)) {
      throw new AppError(
        400,
        "delivery_guarantee must be 'at_least_once' or 'at_most_once'"
      );
    }
  }
  ...
  if (ordering_key !== undefined && ordering_key !== null) {
    if (!["organization_id", "user_id"].includes(ordering_key)) {
      throw new AppError(
        400,
        "ordering_key must be 'organization_id', 'user_id', or null"
      );
    }
  }
  ...
  if (retry_backoff_seconds !== undefined) {
    ...
    if (
      retry_backoff_seconds.some(
        (val) => typeof val !== "number" || val < 0 || val > 3600
      )
    ) {
      throw new AppError(
        400,
...
export function validateUpdateSubscriber(
  req: Request,
  _res: Response,
  next: NextFunction
): void {
