...
export async function handleDeliveryResponse(
  ...
  if (result.success) {
    ...
    if (!canRetry || retriesExhausted) {
      ...
      const backoffIndex = Math.min(message.retry_count, message.retry_backoff_seconds.length - 1);
...
async function checkAndCompleteEvent(eventId: string): Promise<void> {
  ...
  if (result.rows[0]?.all_delivered) {
    await pool.query(`UPDATE events SET status = 'completed', updated_at = NOW() WHERE id = $1`, [
      eventId,
    ]);
